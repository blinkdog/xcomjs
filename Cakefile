...
task 'rebuild', 'Rebuild the module', ->
  clean -> compile -> test -> browserify()

task 'run', 'Run the module', ->
  run()

task 'test', 'Test the module', ->
  clean -> compile -> test()

#----------------------------------------------------------------------------

browserify = (callback) ->
  exec 'node_modules/.bin/browserify main.js > ufo.js', (err, stdout, stderr) ->
    throw err if err
    callback?()
...
run = (callback) ->
  exec 'firefox --new-tab ufo.html', (err, stdout, stderr) ->
    throw err if err
    callback?()
